...
router.post("/requests", requireRole("STUDENT"), visitorController.createVisitorRequest);
router.patch("/requests/:requestId/review", requireRole("WARDEN"), visitorController.reviewVisitorRequest);

router.get("/entry-logs", requireRole("ADMIN", "WARDEN", "STUDENT", "GATEKEEPER"), visitorController.listEntryLogs);
router.post("/entry-logs/check-in", requireRole("GATEKEEPER"), visitorController.recordCheckIn);
